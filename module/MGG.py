...
def _gdbserver(arch: str, ver: BranchProfile, paths: ProjectPaths, config: argparse.Namespace):
  ...
  build_dir = paths.src_dir.gdb / f'build-MGG-{arch}'
  ensure(build_dir)

  destdir = layer_MGG.gdb / f'{arch}-linux-gnu'
...
def build_MGG_tool(arch: str, ver: BranchProfile, paths: ProjectPaths, config: argparse.Namespace):
